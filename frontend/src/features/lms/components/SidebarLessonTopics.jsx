...
export function SidebarLessonTopics({ topics, stepId }) {
    return(
        <>
            {topics.length > 0 && <div className="flex flex-col gap-2">
                {topics.map((topic) => (
                    <Link key={topic.id} className="flex items-center gap-2" to={`/topic/${topic.slug}/`}>
                        <span>
                            {topic.id === stepId ?
