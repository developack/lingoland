...
class TestCommentSerializers(APITestCase):
    ...
    def setUp(self):
        ...
        self.topic_data = {'type': 'Topic', 'slug': 'topic1', 'text': 'test comment'}

    def test_create_comment_serializer_fails_when_content_object_does_not_exist(self):
        data = {'type': 'Article', 'slug': 'wrong-slug', 'text': 'test comment'}
    ...
    def test_not_enrolled_user_cannot_comment_on_lesson(self):
        serializer = CommentCreateSerializer(
            data=self.lesson_data, context={'request': SimpleNamespace(user=self.other_user)})
        with self.assertRaises(ValidationError) as context:
            serializer.is_valid(raise_exception=True)
    ...
    def test_enrolled_user_can_comment_on_lesson(self):
        serializer = CommentCreateSerializer(data=self.lesson_data, context={'request': SimpleNamespace(user=self.user)})
        serializer.is_valid(raise_exception=True)
        self.assertTrue(serializer.validated_data)

    def test_not_enrolled_user_cannot_comment_on_topic(self):
        serializer = CommentCreateSerializer(
            data=self.topic_data, context={'request': SimpleNamespace(user=self.other_user)})
        with self.assertRaises(ValidationError) as context:
            serializer.is_valid(raise_exception=True)
        self.assertIn('Only enroll accounts can submit comments on lessons or topics.', str(context.exception))

    def test_enrolled_user_can_comment_on_topic(self):
        serializer = CommentCreateSerializer(data=self.topic_data, context={'request': SimpleNamespace(user=self.user)})
        serializer.is_valid(raise_exception=True)
        self.assertTrue(serializer.validated_data)

    def test_user_can_comment_on_article_without_enrollment(self):
        data = {'type': 'Article', 'slug': 'article1', 'text': 'test comment'}
        serializer = CommentCreateSerializer(data=data, context={'request': SimpleNamespace(user=self.user)})
        serializer.is_valid(raise_exception=True)
        self.assertTrue(serializer.validated_data)
